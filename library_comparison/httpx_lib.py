...
# === Синхронные функции для каждого запроса ===
...
def post_json_request():
    ...
    with httpx.Client() as client:
        response = client.post(f'{BASE_URL}/post', json=post_data)
    return response
...
def put_request():
    ...
    with httpx.Client() as client:
        response = client.put(f'{BASE_URL}/put', json=put_data)
    return response
...
def get_user_agent():
    ...
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/user-agent', headers=headers)
    return response
...
def post_form_data():
    ...
    with httpx.Client() as client:
        response = client.post(f'{BASE_URL}/post', data=form_data)
    return response
...
def post_raw_text():
    ...
    with httpx.Client() as client:
        response = client.post(f'{BASE_URL}/post', content=raw_text, headers=headers)
    return response
...
def basic_auth_request():
    ...
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/basic-auth/user/pass', 
                            auth=('user', 'pass'))
    return response
...
def digest_auth_request():
    ...
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/digest-auth/auth/user/pass',
...
def set_cookies_request():
    """Установка cookies"""
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/cookies/set?session=abc123')
    return response

@measure_time
def get_cookies_request():
    """Получение cookies через клиент с cookies"""
    # Сначала устанавливаем cookie
    with httpx.Client() as client:
        client.get(f'{BASE_URL}/cookies/set?session=abc123')
...
def error_429_request():
    ...
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/status/429')
    return response
...
def redirect_3_request():
    ...
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/redirect/3')
    return response
...
def redirect_to_request():
    ...
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/redirect-to?url={BASE_URL}/get')
    return response
...
def no_redirect_request():
    ...
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/redirect/1', follow_redirects=False)
    return response
...
def delay_1_request():
    ...
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/delay/1')
    return response
...
def delay_5_timeout_request():
    ...
    try:
        with httpx.Client(timeout=3.0) as client:
            response = client.get(f'{BASE_URL}/delay/5')
        return response
...
def stream_lines_request():
    """Стриминг строк"""
    with httpx.Client() as client:
        with client.stream('GET', f'{BASE_URL}/stream/10') as response:
            stream_lines = []
...
def stream_bytes_request():
    ...
    with httpx.Client() as client:
        with client.stream('GET', f'{BASE_URL}/bytes/1024') as response:
            chunks = []
...
def gzip_request():
    """GZIP декомпрессия"""
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/gzip')
    return response
...
def brotli_request():
    """Brotli декомпрессия"""
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/brotli')
    return response
    ...
    try:
        with httpx.Client() as client:
            with open(temp_file_path, 'rb') as file:
                ...
                response = client.post(f'{BASE_URL}/post', files=files, data=data)
        return response
    finally:
...
def xml_response_request():
    """XML ответ"""
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/xml')
    return response
...
def image_response_request():
    """PNG изображение"""
    with httpx.Client() as client:
        response = client.get(f'{BASE_URL}/image/png')
    return response

@measure_time
def session_operations():
    """Операции с сессией через один клиент"""
    with httpx.Client() as client:
        # Устанавливаем cookie
        client.get(f'{BASE_URL}/cookies/set?session=test')
        
        # Получаем cookies
        response1 = client.get(f'{BASE_URL}/cookies')
        
        # Добавляем постоянные заголовки
        client.headers.update({'X-Session-Header': 'persistent-value'})
        
        # Запрос с постоянными заголовками
        response2 = client.get(f'{BASE_URL}/headers')
...
@measure_time_async
async def async_post_json_request():
...
@measure_time_async
async def async_parallel_delays():
...
async def async_stream_request():
    ...
    async with httpx.AsyncClient() as client:
        async with client.stream('GET', f'{BASE_URL}/stream/5') as response:
            stream_lines = []
...
def test_error_handling():
    ...
    return {
        'error_handling_time': error_time if 'error_time' in locals() else 0
    }
...
def test_sync_parallel():
    """11. Параллельные запросы (синхронные)"""
    print("\n=== 11. Параллельные запросы (синхронные) ===")
...
def test_async_parallel():
    """11a. Асинхронные параллельные запросы"""
    print("\n=== 11a. Асинхронные параллельные запросы ===")
    
    async def run_async_tests():
        # Последовательные асинхронные запросы
        start_time = time.time()
        response1, _ = await async_get_request()
        response2, _ = await async_post_json_request()
        sequential_async_time = time.time() - start_time
        
        # Параллельные асинхронные запросы
        parallel_results, parallel_async_time = await async_parallel_delays()
        
        return {
            'sequential_async_time': sequential_async_time,
            'parallel_async_time': parallel_async_time,
            'parallel_results': parallel_results
        }
    
    results = asyncio.run(run_async_tests())
    print(f"Последовательно (async): {results['sequential_async_time']:.3f}с")
    print(f"Параллельно (async): {results['parallel_async_time']:.3f}с, результаты: {results['parallel_results']}")
...
def test_sessions():
    ...
    (response1, response2), session_time = session_operations()
...
def test_async_streaming():
    """15. Асинхронный стриминг"""
    print("\n=== 15. Асинхронный стриминг ===")
    
    async def run_async_stream():
        (response, stream_lines_count), stream_time = await async_stream_request()
        return response, stream_lines_count, stream_time
    
    response, stream_lines_count, stream_time = asyncio.run(run_async_stream())
    print(f"Асинхронный стриминг: {response.status_code}, время: {stream_time:.3f}с")
    print(f"Получено строк: {stream_lines_count}")
    
    return {
        'async_stream_time': stream_time
    }

def run_all_tests():
    """Запуск всех тестов"""
    print("=== ТЕСТИРОВАНИЕ БИБЛИОТЕКИ HTTPX ===")
    ...
    try:
        ...
        all_results['sync_parallel'] = test_sync_parallel()
        all_results['async_parallel'] = test_async_parallel()
        all_results['upload'] = test_file_upload()
        all_results['formats'] = test_response_formats()
        all_results['sessions'] = test_sessions()
        all_results['async_streaming'] = test_async_streaming()
